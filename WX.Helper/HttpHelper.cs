...
namespace WX.Helper
{
    ...
    public class HttpHelper
    {
        ...
        /// <returns></returns>
        public static bool ValidateCsrfToken(string cookieDomain)
        {
            string csrfToken = GetCsrfToken(cookieDomain);
            string requestCsrfToken = GetContext().Request.Headers["X-CSRF-Token"];
            if (string.IsNullOrWhiteSpace(requestCsrfToken))
            {
                requestCsrfToken = GetContext().Request.Form["csrf-token"];
            }
        ...
        private const string TraceCodeCookieName = "ENMUOUID";
        /// <summary>
        ...
        {
            Guid traceCode;
            var traceCookie = GetContext().Request.Cookies[TraceCodeCookieName];
            if (traceCookie == null
                || string.IsNullOrWhiteSpace(traceCookie.Value)
                || !Guid.TryParse(traceCookie.Value, out traceCode)
            )
            {
                traceCookie = GetContext().Response.Cookies[TraceCodeCookieName];
                ...
                if (string.IsNullOrWhiteSpace(traceCookie.Value) || !Guid.TryParse(traceCookie.Value, out traceCode))
                {
                    ...
                    traceCookie.Expires = DateTime.Now.AddYears(30);
                    if (!string.IsNullOrWhiteSpace(cookieDomain))
                    {
